...
![합성 불꽃(플랫) vs 실제 유류 불꽃(발광)](docs/img/flame_compare.jpg)

*왼쪽: 우리 v1 합성 불꽃(납작한 컷아웃) / 오른쪽: 실제 유류 화재(발광·입체감·주변 조명). 이 차이가 실제 불 전이가 약했던 유력 원인 — 아래 v2에서 아틀라스·발광 합성으로 공략.*
...
**실사 화염 아틀라스(390종)** + **발광 합성**(가산 블렌딩·엣지 페더·조명 스필)으로 공략하는 v2를 설계·사전등록했다.
