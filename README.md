...
3. **헛불(fpr)은 급식실 조리 네거티브로 공략(⑤).** 배포후보의 유일 병목은 **정상 조리에 헛불**(급식실 조리 CCTV fpr 0.26). 예측박스 진단 결과 헛불은 전부 **비화염**(불색 조리음식·스팀·스테인리스 반사)이라, 급식실 조리 영상을 **영상 단위 train/held-out 분리**해 하드네거티브로 재학습(`real_only_grouped_ck`). **안 본 조리영상(held-out) fpr 0.302 → 0.032(약 10× 감소), 일반화 확인.** recall 비용은 작음(장면평균 0.846→0.815, 손실이 가장 약한 흐린불에 집중). → **배포후보 = `real_only_grouped_ck` @ conf 0.25**(recall 장면평균 0.815 · 급식실 헛불 0.032).
4. **시간축 지속성이 프레임 트레이드오프를 이벤트-level에서 해소(B).** 밀집 프레임(0.5s)에 **K-of-N 지속성 경보**(최근 N 중 K 검출 시)를 얹어 재측정: **2-of-3 이 스위트스폿** — 불 이벤트 recall **0.957** · 조리 헛경보율 **0.750→0.125**(6배↓) · 지연 **0.5s**. 진짜 불은 지속돼 recall이 거의 안 깎이고(1.000→0.957), 조리 헛불은 대부분 **순간 깜빡**이라 걸러진다. → **배포 운영점 = `real_only_grouped_ck` · conf 0.25 · 2-of-3 지속성**(event recall 0.957 · 조리 헛경보 0.125 · 지연 0.5s).
...
4. **frame-level 프록시**("불 있나/없나"지 위치·실시간 아님) · **held-out N 작음**(fpr 신뢰구간 큼) · **다른 급식실 일반화 미검증**(수집 데이터 밖 주방).
