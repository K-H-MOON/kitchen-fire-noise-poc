...
**노이즈 강건성 — 그림으로 (검출률과 헛불을 같이 읽는다)**

![검출률: 무너졌다 회복 + 안 배운 노이즈 한계](docs/img/noise_robust_detect.svg)

노이즈가 기존 모델의 검출을 무너뜨리고(가우시안 0.77→0.00), 그 노이즈를 학습하면 회복된다(저조도 0.31→0.79). 회복 폭은 노이즈마다 다르고, **안 배운 노이즈(완전흑백·무작위 가림)엔 일반화가 부분적**이다.
