> 📖 **프로젝트 여정 한눈에(ELI5) → [docs/eli5-summary.html](docs/eli5-summary.html)** — 문제→시도(대부분 실패)→먹힌 것→남은 갭→다음 수를 쉬운 인터랙티브 페이지로 (브라우저로 열기 · 발표/복습용)
>
...
## 🗺️ 도메인 갭 지도 (최신 요약 · 상세 [MEETING §2](docs/MEETING_2026-08-26.md))

**"합성→현실 전이의 갭을 무엇이 닫고 못 닫나"를 시도할 수 있는 만큼 다 돌려 그린 지도.**

| 시도 | 갭에 효과? |
|---|---|
| 실 in-domain 데이터 학습 | ✅ **큼** (recall 0.24→0.85) |
| 조리 특화 하드네거 (⑤) | ✅ (헛불↓ · 배포값 교차-주방 0.066) |
| 시간축 2-of-3 | ✅ (조리 헛경보 6×↓) |
| per-site 보정 | ✅ (현실 경로) |
| 합성 혼합·불꽃 현실성(v2)·표현(v3)·커리큘럼(#2)·도메인 랜덤화(#4) | ❌ |
| **엣지·색 전처리 (#3: sobelb·blend·gray)** | ❌ **전부 RGB에 지배** (recall-matched conf-sweep·색 제거로도 헛불 순감 없음=색-기반 대책 무효) |
| 모델 교체(yolo11) | ⚠️ clean 개선 미확정 |
| 공개 사전학습 모델 · D-Fire 실데이터 추가 | ❌ (전이 안 됨 / 양 아닌 도메인) |
| **범용화**(안 본 급식실 one-model) | ❌ **미해결**(1.465/분 · 남은 최대 갭) |

**한 줄:** 병목은 "데이터 양·모델·불꽃 품질·표현 트릭"이 아니라 **도메인 매칭.** 닫은 건 실 in-domain 데이터 + 조리 특화 음성 + 시간축. → 다음 = **#6 생성형(도메인 매칭 데이터)**. *모든 수치 1-seed·frame-level 경계 — 상세·판정근거는 [MEETING §6.3](docs/MEETING_2026-08-26.md).*
...
**왜 애매해 보이는 소형 불도 모으나 (검수 방어):** 검출기의 실제 약점은 **창백·초기 소형불**(실 test `sc14`, recall→0)이고 큰 불은 이미 잘 잡는다. **화재는 작을 때 잡는 게 안전상 가장 값지다** — 조리로봇 화재 감지의 목적 자체가 초기 진화다. 그래서 소형 초기불을 **의도적으로 모은다**(§8 원칙: "잘 잡는 걸 더 모으면 편향 악화 → 못 잡는 걸 모아야 함"). **판별 기준은 "작아서 제외"가 아니라 "확신 있게 박스 칠 수 있나"**: 식별 가능한 불꽃이면 작아도 유지, 점처럼 애매하거나 정상 조리로 오인되는 것만 제외. 소형불을 반사적으로 빼면 **"큰 불만 잡는 검출기"**가 되어 목적에 역행한다. (제외 기준 두 종류를 구분 — ⓐ 조리 오인=FP 유발이라 **엄격 제외**, ⓑ 너무 작아 라벨 불가=floor 미달만 제외, 그 외 소형불은 **관대하게 유지**.)
...
**v2 (Phase 1 완료 · 음성)** — 이전 저장소 [kitchen-fire-poc](https://github.com/K-H-MOON/kitchen-fire-poc)의 **실사 화염 아틀라스 + 발광 합성**(가산 블렌딩·엣지 페더·조명 스필)으로 병목을 직접 공략했으나, **C3 ≈ C0 (전이 개선 없음 → 병목은 불꽃이 아니었다)**. → 결과: 아래 **v2 결과** 절 · 상세: [docs/PREREGISTER_v2.md](docs/PREREGISTER_v2.md). 값진 음성 → **v3(도메인갭·표현/백본) 전환에 무게.**
...
> 📖 **v1 → v2 → v3 전 과정(데이터·파이프라인 도식·한계·결론)을 한 장으로 → [DETAIL.md](docs/DETAIL.md)** · 미팅용 요약 → [SUMMARY_meeting.md](docs/SUMMARY_meeting.md)
