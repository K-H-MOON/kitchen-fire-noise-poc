...
| 질문 | 현재 근거 | 판정 |
|---|---|---|
| 실제 화재를 잡는가? | 배포후보 `recall 0.813`, 장면 평균 `0.846`; 깨끗한 실화재 13/16 장면은 0.72~1.00 | ✅ 가능성 확인 |
| 정상 조리에 헛불을 내는가? | 조리 네거티브 fine-tune 뒤, 안 본 급식실 frame-level FPR `0.459 → 0.066` (`conf 0.25`) | ✅ 크게 감소 |
