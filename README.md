...
| 생성형 #6 | **실행 완료 · 운영 경로 미채택** | 생성형 사전학습 뒤 실데이터 파인튜닝은 고정 임계값에서 급식실 오탐을 `0.260 → 0.492`로 높였음. `sc14`의 작은 개선은 표본이 작아 미확정이며, 재현율을 맞춘 임계값 비교가 남아 있음 |

생성형 #6의 프롬프트·큐레이션·학습 설계와 평가 결과는 [MEETING §6.3~6.4](docs/MEETING_2026-08-26.md)에 보존합니다. 현재 결과는 생성형 합성이 일반화 문제를 해결했다는 근거가 아니며, 작은 장면의 신호도 추가 검증 전에는 채택하지 않습니다.
...
- **공개용 다음 단계 요약:** [HANDOFF.md](docs/HANDOFF.md)
...
|---|---|---|
| 입력 데이터 | 실제 화재·급식실 조리 영상, 추출 프레임, 가중치는 별도 보관 | 해당 자산의 제공 권한과 분할 매니페스트 |
| 모델·환경 | Google Colab GPU와 `ultralytics` 기반 스크립트 | 당시 패키지·CUDA·런타임의 정확한 고정본은 미기록 |
| 합성·기준선 실행 | `assign_split.py` → 프레임·불꽃 추출 → `colab_synth.py` → `colab_train.py` → 평가 스크립트 | Drive 경로를 제공 가능한 자산 위치로 설정 |
| 실데이터·시간축 실행 | 상세 순서와 판정은 [`docs/HANDOFF.md`](docs/HANDOFF.md) 및 사전 등록 문서에 기록 | 실화재/조리 평가 입력, 학습 가중치, 결과 JSON |
| 생성형 #6 확인 | [`notebooks/gen_ablation.ipynb`](notebooks/gen_ablation.ipynb)와 미팅 기록에 절차·결과 보존 | 생성 데이터, 두 가중치, 급식실·`sc14` 평가 입력; 동일 수치보다 결론의 재현을 확인 |

비공개 자산의 실제 Drive 경로나 계정 정보는 저장소에 적지 않는다. 재실행할 때에는
입력·분할·가중치·결과 JSON의 해시를 함께 기록해, 데이터 접근 권한이 있는 사람이 결과를
추적할 수 있게 한다.
