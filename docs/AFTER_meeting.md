...
> [미팅용 요약(SUMMARY_meeting.md)](SUMMARY_meeting.md)이 v1~v3까지라면, 이 문서는 **회의 후 새 방향의 진행 기록**이다. 결론: v1~v3로 **병목 = 데이터/도메인(합성↔실제 간극)**으로 좁혀졌고, 회의 후 **실제 화재 데이터**로 공략 → **실험 A: 실데이터 학습이 놓침을 없앰**(Indoor recall 0.235→0.985, §5). **누수 통제 재측정 0.899로 견고**(누수 ~8.6점). **도메인 이동 파일럿(§6): 진짜 기름불 test에서 실 0.985 vs 합성 0.523** — **실데이터가 목표 도메인(기름불)에도 전이**(누수 원천 불가한 독립 test). 단 장면 N=4·큰불 위주 소규모라 방향성 신호(초기·작은 불·급식실 시점 미검증).
...
> **⚠️ 남은 경계 = 도메인 (누수는 해소됨):** 누수를 통제한 뒤에도 recall **0.899**로 실데이터 효과는 견고하다. 다만 0.899는 여전히 **같은 도메인(Indoor 일반 실내) test** 성적이다. → 다음 §6에서 **목표 도메인(기름불)** 으로 검증.

**6. 도메인 이동 파일럿 — 진짜 기름/튀김유 화재 test (완료)**
"일반 실화재로 배운 모델이 **목표 위험(기름불)** 도 잡나?" 직접 검증.
...
## 결과 (C) — 도메인 이동 (기름불 파일럿) · frame-level (conf 0.25)
| 모델 | recall | precision | fpr | scene_mean±std |
|---|---:|---:|---:|---:|
| ① 합성-only | 0.523 | 1.000 | 0.000 | 0.563±0.223 |
| ② 실-only | **0.985** | 1.000 | 0.000 | 0.987±0.023 |
| ③ 혼합 | 0.985 | 1.000 | 0.000 | 0.987±0.023 |
| ②g 실-그룹(누수통제) | **0.985** | 1.000 | 0.000 | 0.987±0.023 |

**판정:**
- **실데이터 모델이 진짜 기름불에도 전이** — recall 0.985(64/65), 장면평균 0.987±0.023로 균일. **목표 도메인 첫 실증.** 합성은 0.523(장면 0.32~0.82)으로 약함.
- **③ 혼합 = ②g 그룹 = ② 실** → 합성 무기여 재확인 + **누수통제 모델도 0.985.** 이 test는 학습과 다른 영상이라 **누수 원천 불가** → **0.985는 누수 없는 진짜 목표 성능.** (Indoor 같은-도메인 0.899 → 기름불 목표 0.985: 오히려 높음 = 이 기름불이 크고 선명해 상대적으로 쉬움.)

> **⚠️ 과신 금지:** 장면 **N=4·음성 2장면** → **fpr 0.000·precision 1.0은 약한 증거.** 이 기름불들은 **크고 선명한 발화 후 화염(쉬움)** — **초기·작은 불(조기경보 핵심)은 미검증.** 시연 오버레이 잔존·**급식실 아닌 시연 세트**·정면 카메라 → 실제 급식실 시점·조기 발화는 더 어려울 수 있음. **소규모 파일럿의 방향성 신호**이지 배포 확정 아님.

## 정직한 경계
- **누수는 측정·통제됨(§결과 B).** 랜덤분할 0.985는 약 8.6점 부풀었으나 그룹분할 보정 0.899로 결론(실데이터가 놓침 해소)은 견고. → **남은 경계는 누수가 아니라 도메인.**
- **도메인 이동은 파일럿으로 첫 확인(§6, 기름불 0.985)** 이나 **장면 N=4·큰불·소량 음성** → 초기/작은 불·급식실 시점·다양한 하드네거티브는 미검증. 확정하려면 장면 확대(추가 시연 영상 수동선별·통제 촬영)가 필요.
