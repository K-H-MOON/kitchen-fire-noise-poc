...
- **분할·분량 (실험 A 학습/평가용)**: 우리가 임의로 나눈 게 아니라 **데이터셋(Roboflow export)이 제공하는 train/valid/test 폴더를 그대로 사용** = 랜덤 프레임 단위 **70/15/15 → train 3500 / valid 750 / test 750** (test 내 **fire 392 · nofire 358**). 또한 **fire-only 재구성**: smoke 라벨을 제거하고 fire만 남겨(빈 라벨 = 음성) 합성 모델과 **1클래스로 공정 비교**.
  - **비율은 적절**: 5000장에 70/15/15는 표준. train 3500 = yolov8s 파인튜닝에 충분 · valid 750 = 모델선택/조기종료 기준 · test 750 = 최종 frame-level 지표용(학습·튜닝에 **불가침**). 특히 **val을 "실제" Indoor valid로** 둬서 회의 때 지적된 "합성 val 손해"(모델을 합성 기준으로 고르는 문제)도 해소.
