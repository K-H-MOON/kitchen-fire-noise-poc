...
> **참고 — 평가용 실제 화재가 5편뿐인데 괜찮나?** 영향이 **있다**. 다만 무엇에 영향이 큰지 나눠 봐야 한다.
> - **작은 차이·정확한 %는 못 잰다**: 5개로 평균 내면 오차 범위(신뢰구간)가 넓어, "C0 vs C3의 미세 차이"나 "0.24냐 0.28이냐"를 확정 못 한다. 큰 차이(2배 등)라면 5개로도 보였을 것 → **"큰 효과 없음"은 확실.**
> - **영상 1개가 평균을 흔든다**: 5개뿐이라 유난히 쉽/어려운 영상 하나가 결과를 출렁이게 함(대표성 한계). *참고: 프레임은 수백 장이지만 같은 영상 프레임은 서로 비슷해 "독립 데이터"가 아니므로, 통계적으로는 영상 5개로 친다.*
> - **큰 결론은 견고**: v1·v2·v3 세 번 다 recall이 ~0.25~0.31로 **일관되게 낮아**, "실제 인식률이 약하다"는 방향은 5개로도 안 뒤집힌다. → 결과를 *확정 수치*가 아니라 **방향 지표**로 읽는다.
> - **개수와 별개의 한계**: 이 5편은 **급식실이 아니라 실험실·가정 주방** → "급식실 실화재"를 직접 잰 건 아님(대표성/도메인 문제, 아래 "정직하게 짚을 점" 참고).
> - **고치는 법**: 독립적인 실제 화재 영상/장면을 더 확보하면(배찬우 팀원 Roboflow 제안) 불확실성↓. 단 방향이 이미 분명해 지금은 우선순위 낮춤.
...
- **노이즈 강건성 실험** (실제 CCTV는 흐리거나 어두울 수 있어서): ① 깨끗한 이미지로 배운 모델에 **테스트 때** 노이즈(흐림·저조도·압축)를 점점 세게 얹어 **검출이 얼마나 무너지나** 봄. ② 이번엔 **학습 때** 훈련 이미지에도 **일부러 그 노이즈를 섞어** 다시 학습시켜 **회복되나** 확인. *비유: ①은 깨끗한 교과서로만 공부한 학생이 흐릿한 시험지에 당황(무너짐), ②는 일부러 흐릿한 교과서로도 공부시키면 흐릿한 시험지도 잘 풂(회복).* 통계 신뢰 위해 조건을 바꿔가며 **총 30번 학습**(2개 모델 × 3개 구성 × 5개 seed).
...
**정직하게 짚을 점 (과장 금지):**
- ⚠️ **평가셋 오염 발견(2026-08-24)**: realfire 5편 중 3편(jikken·kanetsu·hisomu)이 **편집된 소방 홍보(PSA) 영상**(자막·PIP·만화 불·타이틀 카드)이고, grease는 **nofire 구간이 오라벨**(실제 불 포함). → **"fire" 라벨 프레임에 실제 불이 없는 경우가 많아 recall이 부당하게 낮게 집계됨.** 즉 **절대 수치(recall 0.31·인식률 %)는 신뢰 불가·과소평가**, 상대 비교(C3≈C0·DINOv3 미미)는 비교적 견고. **→ 깨끗한 평가셋 재확보가 최우선**([DATA_collection_spec §9](DATA_collection_spec.md)).
