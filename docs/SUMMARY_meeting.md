...
**파이프라인**
- **모델**: YOLOv8s(COCO 사전학습) 파인튜닝. 아키텍처 견고성 위해 YOLOv11s로도 미러.
- **학습**: 급식실 배경 + **플랫 4종 불꽃 알파 오버 합성** = 양성 / 배경·하드네거 = 음성. EPOCHS 80.
- **부가**: 노이즈 강건성(강도↑ 저하곡선 → 노이즈 증강으로 회복), 총 30회 학습(v8·v11 × 3구성 × 5seed).
- **검증**: 합성 test · ablation(지름길 여부) · realfire.
