...
- **향후 개선 방향 (우선순위)**: ① **합성 현실성 개선**(가산 블렌딩·글로우·엣지 페더·색보정) — 육안검수상
  합성 불꽃이 "납작한 주황 컷아웃"이라 이게 전이 약함의 유력 원인, 합성 코드만 고치면 되니 값쌈(B-lite).
  ② 그 다음 **불꽃 소재 다양성 확대**(신규 수집, 비쌈·불확실). 근거는 TIMELINE 의 "병목 진단 관찰" 절.
