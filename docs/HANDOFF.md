...
> **★2026-08-24 A안(전이학습) 데이터 수집 1차: 유류/주방 화재 영상 49개 수집→전수 검수(`colab_inspect_firevideos.py`)→깨끗한 실화재 9개(+경계 7)만 생존, 33개 오염 탈락. test 문턱(≥10) 충족·fine-tune 문턱(≥20~25) 미달. ⚠️기존 oilfire_pilot/early가 수집분 v21·v42에서 나온 것으로 확인(=이 둘 학습 금지). 상세 `DATA_collection_spec.md §11`. 다음=현재모델을 새 실화재 test로 재측정(자기교정 순서).**
