...
- **blend**(색 전부보존+엣지 α0.4): 더 민감한 운영점 — recall 0.875↑·**sc14 0.175→0.500(시도 모델 중 창백불 최고 회복)**·scene std 0.126(균일)↑ 이나 **fpr_급식실 0.525로 여전히 2× 악화.** recall 안 맞아(0.875 vs 0.813) fpr 직접비교 운영점 교란 → **conf-match(conf sweep) 필요·미확정.**
- **공통**: 색 버린 sobelb·색 보존 blend 둘 다 fpr ~0.53 → 엣지 정보 자체가 fpr regime 밀어올림(색 손실이 유일 원인 아님). 과적합 없음(둘 다 best=ep60·꺾임 없음).
