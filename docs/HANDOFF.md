...
3. **주황조명 헛불 진단 완료(2026-08-24, `colab_diag_hardneg.py`) → 핵심 다음 = 제품 범위 결정 후 방향 확정.**
   ...
   - **⚠️ 대표성 경계 = 갈림길(사용자 결정 필요)**: 13476222=**고급 레스토랑 무드조명**, 급식실 주방=밝은 산업용 형광등 → 이 헛불은 **급식실엔 out-of-domain**.
     - **(a) 제품이 급식실 전용** → 주황-전구 수집은 **저순위**(out-of-domain 최적화). 진짜 레버 = **배포-대표 데이터**(밝은 급식실 주방·조리·스팀·급식실 실화재) → A안 전이학습(3-③).
     - **(b) 제품이 일반 상업주방(레스토랑 포함)까지** → 따뜻한 조명 **in-domain** → **따뜻한-전구 하드네거 다수 수집→재학습**이 알려진·값싼·확실한 개선책(위 증거로). 로컬 yt-dlp로 다양한 에디슨/장식전구 주방·조리 영상.
   - ③ 이후 A안(전이학습): Indoor 사전학습 → 소량 급식실/유류 fine-tune(아래 원래 4번).
